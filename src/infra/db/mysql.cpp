...
int MySQLStmt::bindTime(int idx, const time_t& value) {
    ...
    // return 0;
    return bindString(idx, TimeUtil::TimeToStr(value));
}
...
time_t MySQLRes::getTime(int idx) {
    ...
    return TimeUtil::StrToTime(m_cur[idx]);
}
...
void MySQLStmtRes::Data::alloc(size_t size) {
    ...
    // 分配用于结果接收的缓冲区，但初始有效长度应为0，
    // 由 MySQL 驱动在 fetch 后通过 length 指针回填真实长度。
    data = new char[size]();
    length = 0;          // 关键修复：避免将最大长度误当作实际长度，导致返回大量\0
    data_length = size;  // 缓冲区容量
}
...
MySQL::ptr MySQLManager::get(const std::string& name) {
    ...
    if (rt->connect()) {
        rt->m_lastUsedTime = time(0);
        return MySQL::ptr(rt,
                          std::bind(&MySQLManager::freeMySQL, this, name, std::placeholders::_1));
    } else {
        delete rt;
        return nullptr;
    }
