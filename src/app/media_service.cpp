...
static auto g_logger = IM_LOG_NAME("app");
...
std::string MediaService::GetStoragePath(const std::string& file_name) {
    time_t now = time(0);
    struct tm t;
    localtime_r(&now, &t);
    char buf[64];
...
std::string MediaService::GetTempPath(const std::string& upload_id) {
    return GetResolvedTempBaseDir() + "/" + upload_id;
}

bool MediaService::InitMultipartUpload(uint64_t user_id, const std::string& file_name,
                                       uint64_t file_size, std::string& out_upload_id,
                                       uint32_t& out_shard_size, std::string* err) {
    out_upload_id = IM::md5(IM::random_string(32) + std::to_string(time(0)));
    out_shard_size = g_shard_size_default->getValue();

    dao::UploadSession session;
    ...
    if (!dao::UploadSessionDAO::Create(session, err)) {
        IM_LOG_ERROR(g_logger) << "create upload session failed: " << (err ? *err : "");
        return false;
    }
...
// 初始化临时目录清理定时器
void MediaService::InitTempCleanupTimer() {
...
bool MediaService::UploadPart(const std::string& upload_id, uint32_t split_index,
                              uint32_t split_num, const std::string& data, std::string* err) {
    dao::UploadSession session;
    if (!dao::UploadSessionDAO::Get(upload_id, session, err)) {
        return false;
    }

    if (session.status != 0) {
        if (err) *err = "upload session is not active";
        return false;
    }
    ...
    if (!existed) {
        ...
        if (!ofs) {
            ...
            if (err) *err = "write part file failed";
            return false;
        }
...
if (!dao::UploadSessionDAO::UpdateUploadedCount(upload_id, (uint32_t)part_count_after, err)) {
    return false;
}
...
if (part_count == (int)session.shard_num) {
    return MergeParts(session, err);
}

return true;
}

bool MediaService::MergeParts(const dao::UploadSession& session, std::string* err) {
    std::string final_path = GetStoragePath(session.file_name);
    std::string dir = IM::FSUtil::Dirname(final_path);
    if (!IM::FSUtil::Mkdir(dir)) {
        if (err) *err = "create storage dir failed";
        return false;
    }

    std::ofstream ofs(final_path, std::ios::binary | std::ios::trunc);
    if (!ofs) {
        if (err) *err = "create final file failed";
        return false;
    }
    ...
    for (uint32_t i = 0; i < session.shard_num; ++i) {
        ...
        if (!ifs) {
            if (err) *err = "read part file failed: " + std::to_string(i);
            return false;
        }
        ofs << ifs.rdbuf();
        ifs.close();
    ...
    if (!dao::MediaDAO::Create(media, err)) {
        return false;
    }

    // 更新 session 状态
    dao::UploadSessionDAO::UpdateStatus(session.upload_id, 1, nullptr);
...
bool MediaService::UploadFile(uint64_t user_id, const std::string& file_name,
                              const std::string& data, dao::MediaFile& out_media,
                              std::string* err) {
    ...
    if (!dao::MediaDAO::Create(out_media, err)) {
        return false;
    }
...
bool MediaService::GetMediaFile(const std::string& media_id, dao::MediaFile& out_media,
                                std::string* err) {
    return dao::MediaDAO::GetById(media_id, out_media, err);
}
