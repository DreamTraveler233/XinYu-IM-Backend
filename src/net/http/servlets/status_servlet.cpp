#include "net/http/servlets/status_servlet.hpp"
...
#define XX(key) ss << std::setw(30) << std::right << key ": "
    ...
    ss << "===================================================" << std::endl;
    ss << "<Logger>" << std::endl;
    ss << LoggerMgr::GetInstance()->toYamlString() << std::endl;
    ss << "===================================================" << std::endl;
    ss << "<Woker>" << std::endl;
    WorkerMgr::GetInstance()->dump(ss) << std::endl;
    ...
    for (auto it = servers.begin(); it != servers.end(); ++it) {
        ...
        for (auto iit = it->second.begin(); iit != it->second.end(); ++iit) {
            ...
            if (!hs) {
                hs = std::dynamic_pointer_cast<HttpServer>(*iit);
            }
