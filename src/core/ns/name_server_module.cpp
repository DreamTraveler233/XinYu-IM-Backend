...
bool NameServerModule::handleRockRequest(RockRequest::ptr request, RockResponse::ptr response,
    ...
    switch (request->getCmd()) {
        ...
        default:
            IM_LOG_WARN(g_logger) << "invalid cmd=0x" << std::hex << request->getCmd();
            break;
    }
    return true;
}

bool NameServerModule::handleRockNotify(RockNotify::ptr notify, RockStream::ptr stream) {
    return true;
}
...
bool NameServerModule::onConnect(Stream::ptr stream) {
    ...
    if (!rockstream) {
        IM_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    }
    auto addr = rockstream->getRemoteAddress();
    if (addr) {
        IM_LOG_INFO(g_logger) << "onConnect: " << *addr;
    }
    return true;
}
...
bool NameServerModule::onDisconnect(Stream::ptr stream) {
    ...
    auto addr = rockstream->getRemoteAddress();
    if (addr) {
        IM_LOG_INFO(g_logger) << "onDisconnect: " << *addr;
    }
...
void NameServerModule::set(RockStream::ptr rs, NSClientInfo::ptr new_value) {
    if (!rs->isConnected()) {
        new_value = nullptr;
    }
    ...
    // WorkerMgr::GetInstance()->schedule("notify",
