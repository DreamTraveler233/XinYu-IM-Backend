...
RockSDLoadBalance::RockSDLoadBalance(IServiceDiscovery::ptr sd) : SDLoadBalance(sd) {}
...
static SocketStream::ptr create_rock_stream(ServiceItemInfo::ptr info) {
    ...
    WorkerMgr::GetInstance()->schedule("service_io", [conn, addr]() {
        conn->connect(addr);
        conn->start();
    });
    return conn;
