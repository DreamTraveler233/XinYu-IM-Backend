...
Uri::ptr Uri::Create(const std::string& uristr) {
    ...
    const char* mark = 0;
    %% write init;
    const char *p = uristr.c_str();     // 解析起始地址
    const char *pe = p + uristr.size(); // 解析结束地址
    const char* eof = pe;
    %% write exec;
...
void Uri::setScheme(const std::string &v) { m_scheme = v; }
void Uri::setUserinfo(const std::string &v) { m_userinfo = v; }
void Uri::setHost(const std::string &v) { m_host = v; }
void Uri::setPath(const std::string &v) { m_path = v; }
void Uri::setQuery(const std::string &v) { m_query = v; }
void Uri::setFragment(const std::string &v) { m_fragment = v; }
void Uri::setPort(int32_t v) { m_port = v; }
}