...
class Dict {
   ...
   public:
    ...
    SharedArray<V> get(const K& k, bool duplicate = true) {
        ...
        RWMutex::ReadLock lock2(s_mutex[pos % MAX_MUTEX]);
    ...
    bool insert(const K& k, const V* v, const uint32_t& size) {
        ...
        RWMutex::ReadLock lock(m_mutex);
        uint32_t pos = hashvalue % m_size;
        RWMutex::WriteLock lock2(s_mutex[pos % MAX_MUTEX]);
    ...
    void foreach (callback cb) {
        ...
        for (size_t i = 0; i < m_size; ++i) {
            RWMutex::ReadLock lock2(s_mutex[i % MAX_MUTEX]);
   ...
   private:
    ...
    static RWMutex s_mutex[MAX_MUTEX];
};

template <class K, class V, class PosHash>
RWMutex Dict<K, V, PosHash>::s_mutex[MAX_MUTEX];
...
class StringDict {
   public:
    static uint64_t GetID(const std::string& str) {
        ...
        return murmur3_hash64(str.c_str(), str.size(), 1060627423, 1050126127);
    }
    static uint64_t GetID(const char* str, const uint32_t& size) {
        if (size == 0) {
            return 0;
        }
        return murmur3_hash64(str, size, 1060627423, 1050126127);
    }
    static uint64_t GetID(const char* str) {
        if (str == nullptr) {
            return 0;
        }
        return murmur3_hash64(str, 1060627423, 1050126127);
    }
    ...
    bool readFrom(std::istream& is, uint64_t speed = -1) { return m_dict.readFrom(is, speed); }

    bool writeTo(std::ostream& os, uint64_t speed = -1) { return m_dict.writeTo(os, speed); }
