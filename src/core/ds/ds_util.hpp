...
class Murmur3Hash<std::vector<T>, seed> {
   public:
    uint32_t operator()(const std::vector<T>& data) {
        return murmur3_hash(&data[0], data.size() * sizeof(T), seed);
    }
};
...
class Murmur3Hash64<std::vector<T>, seed, seed2> {
   public:
    uint32_t operator()(const std::vector<T>& data) {
        return murmur3_hash64(&data[0], data.size() * sizeof(T), seed, seed2);
    }
};
...
struct Ivt {
    ...
    inline void duplicate(const std::vector<V>& v) { duplicate(&v[0], v.size()); }
