...
void Bitmap::writeTo(ByteArray::ptr ba) const {
...
bool Bitmap::operator==(const Bitmap& b) const {
    ...
    if (m_compress != b.m_compress || m_size != b.m_size || m_dataSize != b.m_dataSize) {
        return false;
    }
    ...
    if (left) {
        ...
        return (m_data[m_dataSize - 1] & MASK[left]) == (b.m_data[b.m_dataSize - 1] & MASK[left]);
    }
    return true;
}
            ...
            if (!tmp) {
                ...
                if (i == start) {
                    if (m_size % VALUE_SIZE == 0) {
                        ...
                        cur_pos += 1;
                        for (int32_t n = m_size % VALUE_SIZE; n >= 0; --n, --cur_pos) {
                            if (tmp & POS[n]) {  //((base_type)1 << n)) {
                                if (!cb(cur_pos)) {
                                    return;
                                }
