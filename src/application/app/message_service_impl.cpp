...
bool MessageServiceImpl::buildRecord(const model::Message& msg, dto::MessageRecord& out,
                                     std::string* err) {
    out.msg_id = msg.id;
...
Result<dto::MessagePage> MessageServiceImpl::LoadRecords(const uint64_t current_user_id,
...
Result<dto::MessagePage> MessageServiceImpl::LoadHistoryRecords(const uint64_t current_user_id,
                                                                const uint8_t talk_mode,
...
Result<void> MessageServiceImpl::DeleteMessages(const uint64_t current_user_id,
...
Result<void> MessageServiceImpl::DeleteAllMessagesInTalkForUser(const uint64_t current_user_id,
                                                                const uint8_t talk_mode,
                                                                const uint64_t to_from_id) {
    Result<void> result;
    std::string err;
    ...
    if (!m_message_repo->MarkAllMessagesDeletedByUserInTalk(db, talk_id, current_user_id, &err)) {
        if (!err.empty()) {
            trans->rollback();
            IM_LOG_WARN(g_logger) << "MarkAllMessagesDeletedByUserInTalk failed, talk_id="
                                  << talk_id << ", err=" << err;
            result.code = 500;
            result.err = "删除消息失败";
            return result;
...
Result<void> MessageServiceImpl::ClearTalkRecords(const uint64_t current_user_id,
                                                  const uint8_t talk_mode,
                                                  const uint64_t to_from_id) {
    Result<void> result;
    std::string err;
...
Result<void> MessageServiceImpl::RevokeMessage(const uint64_t current_user_id,
                                               const uint8_t talk_mode, const uint64_t to_from_id,
    ...
    if (!m_message_repo->GetById(msg_id, message, &err)) {
        if (!err.empty()) {
            IM_LOG_WARN(g_logger) << "RevokeMessage GetById error msg_id=" << msg_id
                                  << " err=" << err;
            result.code = 500;
            result.err = "消息加载失败";
            return result;
...
Result<void> MessageServiceImpl::UpdateMessageStatus(const uint64_t current_user_id,
                                                     const uint8_t talk_mode,
                                                     const uint64_t to_from_id,
...
Result<dto::MessageRecord> MessageServiceImpl::SendMessage(
    ...
    if (talk_mode == 1) {
        IM::dto::ContactDetails receiver_view;
        // 查询接收者视角下是否仍是好友：owner=接收者, friend=发送者
        if (!m_contact_repo->GetByOwnerAndTarget(db, to_from_id, current_user_id, receiver_view,
                                                 &err) ||
    ...
    if (talk_mode == 1) {
        ...
        try {
            ...
            (void)m_contact_repo->GetByOwnerAndTarget(db, current_user_id, m.receiver_id, cd_sender,
                                                      &cerr_sender);
            ...
            if (deliver_to_receiver) {
                ...
                (void)m_contact_repo->GetByOwnerAndTarget(db, m.receiver_id, current_user_id,
                                                          cd_receiver, &cerr_receiver);
    ...
    if (!m_talk_repo->bumpOnNewMessage(db, talk_id, current_user_id, m.id,
                                       static_cast<uint16_t>(m.msg_type), last_msg_digest, &err)) {
        if (!err.empty()) {
            trans->rollback();
...
bool MessageServiceImpl::GetTalkId(const uint64_t current_user_id, const uint8_t talk_mode,
